...
with open("2021_market.txt", "w") as file:
    ...
    for i in range(numeroTotalDeDados):
        ...
        boughtDate = (datetime(2021,1,1) + timedelta(days=random.randint(0,364))).strftime("%Y-%m-%d")

        file.write('{}, {}, {}, {}\n'.format(contadorDeIteracao, clientID, '#'.join(productBought), boughtDate))

# Ve se o arquivo foi criado corretamente
if os.path.exists("2021_market.txt") and (contadorDeIteracao == numeroTotalDeDados):
